...
import { updateDoc, doc, arrayUnion, arrayRemove, getDoc, setDoc, writeBatch } from 'firebase/firestore';
import { Sparkles, LayoutList, CalendarClock, FileJson, X, Users, Save, Briefcase, Activity } from 'lucide-react';
...
const AdminPanel = ({ teamData, staffLoads }) => {
    // --- TABS STATE ---
    const [activeTab, setActiveTab] = useState('OPERATIONS'); // 'OPERATIONS' or 'WELLBEING'
    ...
    return (
        <div className="monday-card p-6 mt-6 mb-12 dark:bg-slate-800 dark:border-slate-700">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 border-b border-slate-200 dark:border-slate-700 pb-6">
                <div>
                    <h2 className="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tight">Admin Control Center</h2>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">
                        {activeTab === 'OPERATIONS' ? 'Workload, Projects & Attendance' : 'Staff Wellbeing & Engagement'}
                    </p>
                </div>
                ...
                <div className="flex bg-slate-100 dark:bg-slate-900 p-1 rounded-lg">
                    <button 
                        ...
                        className={`flex items-center gap-2 px-6 py-2 rounded-md text-xs font-bold uppercase transition-all ${activeTab === 'OPERATIONS' ? 'bg-white dark:bg-slate-700 shadow text-indigo-600 dark:text-white' : 'text-slate-500'}`}
                    >
                        <Briefcase size={14} /> Operations
                    </button>
                    <button 
                        onClick={() => setActiveTab('WELLBEING')}
                        className={`flex items-center gap-2 px-6 py-2 rounded-md text-xs font-bold uppercase transition-all ${activeTab === 'WELLBEING' ? 'bg-white dark:bg-slate-700 shadow text-emerald-600 dark:text-emerald-400' : 'text-slate-500'}`}
                    >
