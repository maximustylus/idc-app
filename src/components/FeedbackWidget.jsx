...
const FeedbackWidget = ({ user }) => {
    ...
    // 📡 THE LISTENER: Waits silently for AURA to trigger it
    useEffect(() => {
        const handleOpenRequest = () => setIsOpen(true);
        
        window.addEventListener('open-bug-report', handleOpenRequest);
        return () => window.removeEventListener('open-bug-report', handleOpenRequest);
    }, []);
    ...
    // 🛡️ UX UPGRADE: If it's not open, render absolutely nothing. No floating button!
                ...
                {success ? (
                    <div className="p-12 text-center">
                        <div className="w-16 h-16 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-4">
                            <span className="animate-in zoom-in duration-500 delay-150">
                                <Send size={32} />
                            </span>
                        </div>
                    ...
                    <form onSubmit={handleSubmit} className="p-6">
                        <div className="flex gap-2 mb-6">
                            <button type="button" onClick={() => setType('bug')} className={`flex-1 py-3 px-4 rounded-xl flex items-center justify-center gap-2 text-xs font-bold uppercase transition-colors ${type === 'bug' ? 'bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-400 border-2 border-red-500' : 'bg-slate-50 dark:bg-slate-800 text-slate-500 border-2 border-transparent hover:bg-slate-100 dark:hover:bg-slate-700'}`}>
                                <Bug size={16} /> Bug
                            </button>
                            <button type="button" onClick={() => setType('idea')} className={`flex-1 py-3 px-4 rounded-xl flex items-center justify-center gap-2 text-xs font-bold uppercase transition-colors ${type === 'idea' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400 border-2 border-amber-500' : 'bg-slate-50 dark:bg-slate-800 text-slate-500 border-2 border-transparent hover:bg-slate-100 dark:hover:bg-slate-700'}`}>
                        ...
                        <button 
                            ...
                            className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-black rounded-xl uppercase transition-colors shadow-lg flex items-center justify-center gap-2 active:scale-[0.98]"
                        >
